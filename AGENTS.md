...
4. `Tools/FairyGUI/Run Validation Suites` —— 跑仓库内的 282 项行为/像素/不变量套件
   （需已在 Play 模式；无头形态见下）。
...
- **后端覆盖：两条都要跑**。历史上本机编辑器的 buffer 路径（顶点级 StructuredBuffer）
  ...
  （对照实验确认像素来自实例 draw），buffer 后端 227/227 全绿，双后端 454/454（批5b 并入后 282 项，双后端 564/564）。
...
- **验证套件已全部固化进仓库**：`Assets/Examples/InstancedPoC/Validation/`
  （19 套 282 项：M1 重组器 17、M3 裁剪栈 10、M7 SDF 17、M4 场景 19、批1-4
