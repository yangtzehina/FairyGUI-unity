...
全绿,双后端合计 **454/454**（自动挂载套件并入后 247 项/双后端 **494/494** 复验全绿）。
...
```
INSTANCED VALIDATION VERDICT: PASS pass=247 fail=0
```
...
| `FqsAutoMountSuite` | 20 | CreateObject 自动挂载：装填/兑现时序、provider 缓存、defer 阈值、嵌套作用域、**像素对照 diff=0**、陈旧回退。其中 **8 项是以对抗审查确认缺陷命名的回归项**：删除内容不留幽灵（构造期绑定的核心缺陷）、id 定身份、非导出组件不查 blob、分支解析键、contentScaleLevel 门、无源哈希拒绝、实例级拒绝不锁定、Bake 恢复调用方 mount |
...
**必须单独一个 Unity 进程跑**,不能接在 247 项行为套件后面——那 247 项自己就会留下
