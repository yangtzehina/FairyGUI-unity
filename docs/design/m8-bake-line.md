...
- **命名**：`BlobFileName(item, level)`——档位 0 保持裸名（既有 blob 全部
  视为 0 档，零迁移），档位 N 追加 `_sN` 后缀，各档位产物共存一个目录。
- **烘焙**：`FqsBakeMenu` 按烘焙时 `GRoot.contentScaleLevel` 写带后缀的文件。
  给每个目标档位出一套产物 = `UIContentScaler.scaleLevel = N`（可写静态量）
  → **重载包**（档位在包加载时决定选哪套图集项，先设后载才有效）→ 重跑
  Bake。每档一跑的矩阵可以脚本化进 CI；菜单本身不做一键全出（重载包会拆掉
  当前会话的活 UI，留给调用方决定时机）。
- **运行时**：默认 provider 先按当前档位取 `_sN` 文件，miss 再回退裸名。
  回退**不可能画错档美术**——挂载门仍然比对烘入档位，档位不符照拒，行为
  与完全没有文件一致；回退只是让单档位项目与老产物零成本继续工作。
- **拒挂日志**升级为可操作指引（怎么补烘该档位）。自定义 provider 的项目
  自行决定按档位取哪个文件，门在 `FqsMount.TryMount` 层，绕不过。
