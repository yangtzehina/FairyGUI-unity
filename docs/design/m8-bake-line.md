...
### 多档位发布（2026-08-08，审计 MED 的配套故事）

档位门（上表第三行）此前只有"拒"没有"配"：blob 文件名不含档位，一套
Resources 目录只装得下一个档位的产物——多档位发布的项目在非烘焙档位的整类
设备上全部静默拒挂，且遥测分不清"没烘"和"档位不符"。补齐后的完整形态：

- **命名**：`BlobFileName(item, level)`——档位 0 保持裸名（既有 blob 全部
  视为 0 档，零迁移），档位 N 追加 `.sN` 后缀（'.' 在消毒字母表之外，且 id
  与名字同规则消毒——(name,id,level) 三元组构造性不碰撞），各档位产物共存
  一个目录。
- **烘焙**：`FqsBakeMenu` 按烘焙时 `GRoot.contentScaleLevel` 写带后缀的文件，
  汇总日志回显档位、非 0 档 LogWarning（超大 Game view 静默烘出 .s1 套件曾
  与"没烘过"在手机上不可区分）。给每个目标档位出一套产物 =
  `UIContentScaler.scaleLevel = N`（可写静态量）→ **重载包** → 重跑 Bake。
  机制精确说：档位是组件**构造时活读**的（`PackageItem.getHighResolution`
  每次现读静态量），重载包的真实作用是清掉已按旧档位物化的 item 资产（位图
  字体等）；且设完档位到烘完之间**不得触发屏幕尺寸事件**——
  `Stage.HandleScreenSizeChanged → UIContentScaler.UpdateScaleLevel` 会按
  当前分辨率把档位改回去。每档一跑的矩阵可脚本化进 CI；菜单不做一键全出。
- **运行时**：默认 provider 先按当前档位取 `_sN` 文件，miss 再回退裸名。
  回退**不可能画错档美术**——挂载门仍然比对烘入档位，档位不符照拒，行为
  与完全没有文件一致；回退只是让单档位项目与老产物零成本继续工作。
- **拒挂日志**升级为可操作指引（怎么补烘该档位），与本节流程同一套话术。
  自定义 provider 的项目自行决定按档位取哪个文件，门在 Mount 层，绕不过；
  默认 provider 的缓存条目记录取数档位，运行时变档（窗口缩放重推导）自动
  重新取数，不会拿旧档字节在门上撞到天荒地老。

**FormatVersion 3（2026-08-08）**：重组器改拒四角渐变后，v2 blob 里可能存有
按旧逻辑压扁颜色的渐变 quad——静默挂载会永久渲染错误像素，故 bump 版本让
全部旧 blob 干净拒载（回退运行时 Extract，只慢不错）。**含四角渐变/渐变文本
的组件从此拒烘**；BakeAll 对拒烘组件会顺手删除其当前档位的旧 blob 文件，并
在每包结束时打印孤儿文件清单（改名/删除/取消导出留下的尸体）。
...
## 4.8 全页超集烘焙（M8-7，2026-08-02）

默认页烘焙的缺口：带 GearDisplay 的组件里被隐藏的页不进 blob，运行时翻页触发
「blob 缺席 → 优雅失效 → 整 mount 回退运行时」。**按钮自己的按下态就是控制器页**——
一次普通点击就能让挂载组件降级。M8-7 关掉这个缺口：

- **烘焙侧**：提取前把子树里所有隐藏对象临时置可见（走 property setter 而非裸标志——
  SetActive 必须触发才能让下一次 update 把网格建出来，还原时也要走同一套副作用）、
  `Stage.ForceUpdate()` 建网格、提取、finally 里精确还原动过的对象。renderingOrder 是
  逐帧瞬态，不残留——**套件用字节级重烘一致性直接断言还原纯度**（s2），立项书里
  "还原完整性"的担忧从论证变成了断言。开关 `FqsBaker.supersetVisibility`（默认开）。
- **拼接侧零改动**：M8-2/4 的无状态可见性重放本来就每次 splice 从活标志重推
  （`HideSubtree`/`HideInvisibleBelow`），超集 blob 里当前隐藏的段区自动清零；
  `MapPaths` 按索引全量走、不看可见性，绑定天然支持超集。
- **对拍门超集感知**：mounted 侧会多出清零 quad 与叶范围——几何对比两侧对称过滤
  零面积 quad，叶数放宽为 runtime ≤ mounted（像素门不变，仍是 diff==0）。
- **既有测试的前提修正**：M8-4 g16 测的是「缺席内容显示 → 优雅失效」，超集默认开后
  内容不再缺席——该 fixture 现在钉死 `supersetVisibility = false` 烘焙（降级阶梯照旧
  被 s7 与 g16 双向覆盖）。

验收（`FqsSupersetSuite` 13 项）：超集确实进 blob（2→4 叶）、字节级重烘一致、
**首次显示从未渲染过的内容零重编译**、六连翻页零重编译且逐状态像素与运行时孪生
全等（mean=0.000）、孪生被动触发的重拼接不破坏 mount；**命中测试三件套**——当前页
按钮在 `Stage.HitTest`（真实触摸的同一解析路径）下命中且 BubbleEvent 到达监听器、
隐藏页按钮在其位置**不**命中、翻页后角色精确互换；超集关闭时缺席显示仍优雅降级。

测量方法注记：零重编译断言只围住**挂载实例**的翻页——运行时孪生每次翻页本来就要
重编译，那是运行时走树的固有价格、恰是本站要消掉的差异；第一版套件把两者放同一
窗口测总数，把孪生的账记到了 mount 头上（隔离诊断：挂载侧 +0/+0/+0，孪生侧
+1/+1/+1）。
...
| 2026-08-02 | M8-7 全页超集烘焙 | ✅ | 见 §4.8：烘焙临时置可见 + 既有无状态重放清零；翻页/按钮按下态零重编译；命中测试三件套入门禁 |
